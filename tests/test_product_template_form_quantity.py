...
class TestCase(ModuleTestCase):
    ...
    def test0010get_lag_quantity(self):
        ...
        InstallModule = pool.get('ir.module.install_upgrade', type='wizard')

        # Install stock_lot
        with Transaction().new_transaction() as transaction:
            stock_lot_jreport_module = Module.search([
                    ('name', '=', 'stock_lot_jreport')
                    ])
            Module.install(stock_lot_jreport_module)
            transaction.commit()
            session_id, _, _ = InstallModule.create()
            transaction.commit()
            install_module = InstallModule(session_id)
            install_module.transition_upgrade()
            InstallModule.delete(session_id)
            transaction.commit()
